...
void getUniqueEntriesAttrs(vector<vector<int>> &exactAttrMatches, vector<string> & uattrStrs, int targetAttr) {
	uattrStrs.resize(exactAttrMatches.size());

	for (size_t i = 0; i < exactAttrMatches.size(); i++) {
		uattrStrs[i] = uniqueRecords[exactAttrMatches[i][0]][targetAttr];
    }
}
...
void processAllBlocks(BlockingMethods &bm, vector<vector<int>> &parentArrVec){
	for(int i=0; i<bm.boundaryArr.size(); i++){
		...
		for(int j=start; j<start+range-1; j++){
			for(int k=j+1; k<start+range; k++){
				...
				int recid_k = bm.blockingIDList[k].second;
				if(!uf.isConnected(recid_j, recid_k)){
				 	if(recCom.isLinkageOk(uniqueRecords[recid_j], uniqueRecords[recid_k], recid_j, recid_k, parentArrVec)) {
						uf.weightedUnion(recid_j, recid_k);
				 		totalEdges++;
				 	}
				}
...
void processAllAttrBlocks(BlockingMethods &bm, UnionFind &uf_attr, RecordComparator &recCom_attr, vector<string> &attrVals){
	for(int i=0; i<bm.boundaryArr.size(); i++){
		int start = bm.boundaryArr[i].first;
		int range = bm.boundaryArr[i].second;
		for(int j=start; j<start+range-1; j++){
			for(int k=j+1; k<start+range; k++){
				int recid_j = bm.blockingIDList[j].second;
				int recid_k = bm.blockingIDList[k].second;
				if(!uf_attr.isConnected(recid_j, recid_k)){
				 	if(recCom_attr.isLinkageOk(attrVals[recid_j], attrVals[recid_k])) {
						uf_attr.weightedUnion(recid_j, recid_k);
				 	}
				}
			}
		}
...
void doSortedComp(vector<vector<int>> &parentArrVec) {
	...
	for (int i = 0; i < headlessCopies.size()-1; i++) {
		...
		while (headlessCopies[i].second.compare(headlessCopies[i+j].second) == 0) {
            ...
            if (!uf.isConnected(recID_i, recID_j)) {
                if(recCom.isLinkageOk(uniqueRecords[recID_i], uniqueRecords[recID_j], recID_i, recID_j, parentArrVec)) {
...
void findAttrConnComp(int totalEntries, UnionFind &uf_attr, map<int, vector<int>> &attrConnComp)
{
	int root;
	...
	{
		root = uf_attr.find(i);
		attrConnComp[root].push_back(i);
	}

    cout<< " Single Linkage Connected Components: " << attrConnComp.size()<<endl;
}
...
void findFinalConnectedComp() {
    ...
    for (auto const& p : approxConnectedComponents) {
		// cout<< "Cluster " << pairsAccessed << " size " << p.second.size() << endl;
        pairsAccessed++;
...
int main(int argc, char** argv) {
	...
	// getCombinedData_noPad(vec2D, combinedData, lenMax);
	...
	// radixSort_utility_uneven_improved(lenMax, combinedData);
	// radixSort_utility(lenMax, combinedData);
	...
	// Do single linkage for each attribute

	vector<vector<int>> recParentArrAttrs;
	recParentArrAttrs.resize(attributes);
	for(int i=0; i< attributes; i++){
		recParentArrAttrs[i].resize(totalUniqueRecords);
	}

	// single linkage for each attribute
	for(int targetAttr = 1; targetAttr<attributes; targetAttr++){

		vector<pair<int, string> > recIndAttrValPairs;
		vector<pair<int, vector<int>>> uattrIndRecIndList;
		vector<vector<int> > exactAttrMatches;
		vector<string> uattrStrs;
		// aligns the target attribute values to max value length for that attribute
		getCombinedData_attr(uniqueRecords, recIndAttrValPairs, lenMax, targetAttr);
		// sort the target attributes
		radixSort_std(lenMax, recIndAttrValPairs);

		int totalUniqueAttrs;
		// finds unique target attribute values. 
		// Each entry is a vector of unique record ids that share same target attribute value
		getExactAttrs(exactAttrMatches, recIndAttrValPairs);
		cout << "total exact clusters for attr :" << targetAttr << " is: " << exactAttrMatches.size() << endl;
		// gets unique target attribute value strings
		getUniqueEntriesAttrs(exactAttrMatches, uattrStrs, targetAttr);

		UnionFind ufAttr;
		ufAttr.setVariable(exactAttrMatches.size());
		BlockingMethods bm;

		bm.setBlocking(KMER, alphabetSize, uattrStrs);
		bm.getNormalBlockingIDArrayAttrs();
		// bm.getSuperBlockingIDArray();
		bm.sortBlockingIDArray();
		bm.removeRedundentBlockingID();
		bm.findBlockBoundaries();
		processAllAttrBlocks(bm, ufAttr, recCom, uattrStrs);
		map<int, vector<int>> attrConnComp;
		findAttrConnComp(exactAttrMatches.size(), ufAttr, attrConnComp);
		int clusterID = 0;
		for (auto const& p : attrConnComp){
			for(int i=0; i<p.second.size(); i++){
				for(int j=0; j<exactAttrMatches[p.second[i]].size(); j++){
					recParentArrAttrs[targetAttr][exactAttrMatches[p.second[i]][j]] = clusterID;
				}
			}
			clusterID++;
		}
	...
	processAllBlocks(bm, recParentArrAttrs);
	...
	doSortedComp(recParentArrAttrs);
	...
	string outname = "Out_" + std::to_string(world) + "_"+ argv[1] + "_tokenBased.csv";
